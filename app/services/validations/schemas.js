...
}).required().length(2); // Les deux champs (password et newPassword) sont requis

// Schéma de validation pour la demande de réinitialisation du mot de passe
export const contactSchema = Joi.object({
  email: Joi.string().email(), // L'email doit être une adresse email valide
  message: Joi.string().min(1).required(), // Le message doit être une chaîne non vide et est requis
}).required(); // Le champ email est requis
